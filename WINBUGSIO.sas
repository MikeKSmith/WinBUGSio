***************************************************************************
*                                                                         *
*     Program Name       : winbugsio_v001.sas                             *
*                                                                         *
*     System             : Windows SAS v8.2                               *
*                                                                         *
*     Program's function : SAS code for running WinBUGS and handling I/O  *
*                          with SAS.                                      *
*                                                                         *
*     Assumptions        : Valid model file is available check this using *              
*                          scripts within WinBUGS first                   *
*                                                                         *
...
*-------------+--------------------+--------------------------------------*
* 07-Sep-05   | Helen Richardson   | Updated original macro WINBUGSIO.SAS *
*             |                    | written by Mike K Smith to include   *
*             |                    | requirements specified in functional *
*             |                    | specification.                       *
*-------------+--------------------+--------------------------------------*
* 08-Sep-05   | Helen Richardson   | Version T001 Testing                 *
*-------------+--------------------+--------------------------------------*
* 29-Sep-05   | Helen Richardson   | UAT updates:                         *
*             |                    | Batch file written to winbugs rootdir*
*             |                    | Add quit to batch file               *
*             |                    | log and window statement in batch    *
*             |                    | file should be in single quotes      *
*             |                    | Add cmpres function to variable lists*
*             |                    | to remove blanks                     *
*             |                    | Translate ] to blank in codafile     *
*             |                    | Delete coda dataset                  *
*             |                    | Explicitly check codavar list against*
*             |                    | node list                            *
*             |                    | Additional functionality added to    *
*             |                    | allow user to specify external files *
*             |                    | for datafile1 and datafile2          *
*-------------+--------------------+--------------------------------------*
* 20-Oct-05   | Helen Richardson   | Copy of ../testing/macro/            *
*             |                    | winbugsio_t002.sas.  Winbug call     *
*             |                    | uncommented.                         *
*             |                    | Version V001 Release                 *
*-------------+--------------------+--------------------------------------*
***************************************************************************;
...
filename fileout2 "c:\program files\winbugs14\&batchfile..txt";
data _null_;
   file fileout2;
   put@1 "display('log')";
   put@1 "check('&wbdir.&modelfile..txt')";
   %if %bquote(%upcase(&dsvar)) eq EXTERNAL %then %do;
     %if &dirsp1=0 and &dataflag1 %then put@1 "data('&wbdir.&datafile1')";
     %else %if &dirsp1=1 and &dataflag1 %then put@1 "data('&datafile1')";;
   %end;
   %else %if %bquote(%upcase(&dsvar)) ne EXTERNAL  and "&dsvar" ne "" %then %do;
     put@1 "data('&wbdir.&datafile1..txt')";
   %end;
   %if %bquote(%upcase(&constlist)) eq EXTERNAL %then %do;
     %if &dirsp2=0 and &dataflag2 %then put@1 "data('&wbdir.&datafile2')";
     %else %if &dirsp2=1 and &dataflag2 %then put@1 "data('&datafile2')";;
   %end;
   %else %if %bquote(%upcase(&constlist)) ne EXTERNAL  and "&constlist" ne "" %then %do;
     put@1 "data('&wbdir.&datafile2..txt')";
   %end;
   put@1 "compile(&filetot)";
   %do i=1 %to &filetot;
      put@1 "inits(&i,'&wbdir.&&file&i...txt')";
   %end;
   put@1 "gen.inits()";
   put@1 "update(&burnin)";
   %if "&nodes" ne "" %then %do;
      ...
      %end;
   %end;
   put@1 "update(&samples)";
   put@1 "stats(*)";
   put@1 "save('&wbdir.&logfile..txt')";
   %if "&codavar" eq "*" %then %do;
      put@1 "coda('*','&wbdir.coda')";
   %end;
   %else %if "&codavar" ne "" and "&codaerr" ne "Y" %then %do;
      %do i=1 %to &codavartot;
         put@1 "coda('&&codavar&i','&wbdir.&&codavar&i')";
      %end;
   %end;
   put@1 "display('window')";
   put@1 'quit()';
run;
proc printto; run;
...
x cd c:\program files\winbugs14\;
x winbugs14 /PAR &batchfile..txt;
...
data &dsout;
  infile "&dir&logfile..txt" firstobs=&i obs=&j expandtabs truncover;
  length var $20;
...
%do i=1 %to &codavartot;
   ...
   %do j=1 %to &total;
      data &&parm&j;
         infile "&dir.&&codavar&i..1.txt" expandtabs truncover firstobs=&&start&j obs=&&stop&j;
         input iteration value ;
	     parameter="&&parm&j";
      run;
      %if "&codavar" eq "*" %then %do;
         proc append base=coda data=&&parm&j;
         run;
